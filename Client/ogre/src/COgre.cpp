...
void COgre::init()
{
    ...
    Ogre::Window* renderWindow = root->createRenderWindow("OgreTest", 1280, 720, false, &params);

    // Create the SceneManager
    sceneManager = root->createSceneManager(Ogre::ST_GENERIC, 4, "SceneManager");
    ...
    //root->getCompositorManager2()->createBasicWorkspaceDef("sceneworkspace", Ogre::ColourValue::Blue);
    //Ogre::CompositorWorkspace* workspace =
    //    root->getCompositorManager2()->addWorkspace(sceneManager, renderWindow->getTexture(), mainCamera, "sceneworkspace", true);

    //// register HLMS
    //Ogre::String rootHlmsFolder = "I:/ogre-next-master/Scripts/BuildScripts/output/Ogre/ogre-next/Samples/Media/";
    ...
    //    mHlmsPbs->createDatablock(datablockName, datablockName, Ogre::HlmsMacroblock(), Ogre::HlmsBlendblock(), Ogre::HlmsParamVec()));
    ...
    //    "grid.png", Ogre::GpuPageOutStrategy::Discard, Ogre::TextureFlags::AutomaticBatching | Ogre::TextureFlags::PrefersLoadingFromFileAsSRGB,
    ...
    Ogre::HlmsUnlitDatablock* datablock = static_cast<Ogre::HlmsUnlitDatablock*>(
        Ogre::Root::getSingletonPtr()
            ->getHlmsManager()
            ->getHlms(Ogre::HLMS_UNLIT)
            ->createDatablock("particleMat", "particleMat", Ogre::HlmsMacroblock(), Ogre::HlmsBlendblock(), Ogre::HlmsParamVec()));
...
bool COgre::frameRenderingQueued(const Ogre::FrameEvent& evt)
{
